...
Compiles and runs the programs under tests/benchmarks/publish/ in Flow, C, Rust,
and Python, then writes tests/benchmarks/RESULTS.md.
...
Usage:
  python3 tests/benchmarks/run_publish.py            # full run, writes RESULTS.md
  python3 tests/benchmarks/run_publish.py --quick    # 1 repetition, no report
"""
...
def write_report(data, compile_times, env, reps, have_rust, out_path):
    ...
    add("  every language. Sources live in tests/benchmarks/publish/.")
    ...
    add("```")
    add("")
    add("This regenerates tests/benchmarks/RESULTS.md in place. A full run takes")
