...
class TestEndToEnd:
    ...
    def test_reset_writes_are_simultaneous(self, tmp_path):
        # `a becomes b; b becomes a` must swap, exactly like `evolves as`
        # derivative evaluation. Sequential assignment would give a == b.
        # The clock hits the threshold exactly once, so a re-firing guard
        # would swap back and also fail.
        program = """
extern { function printf(fmt: string, val: f64) -> i32 }
...
"""
        out = run_flow_program(program, tmp_path, "swap_e2e")
        a, b = (float(line) for line in out.split())
        assert a == 2.0
        assert b == 1.0

    @pytest.mark.skipif(shutil.which("clang") is None, reason="clang not found")
    def test_bouncing_ball_decays(self, tmp_path):
        # Drop from 2 m with restitution 0.8: successive rebound peaks
        # should shrink by roughly restitution squared (0.64). Detection is
        # a sign-change guard at dt resolution, so the tolerance is loose.
        program = BALL + """
extern { function printf(fmt: string, val: f64) -> i32 }
...
"""
        out = run_flow_program(program, tmp_path, "ball_e2e")
        lines = out.strip().split("\n")
        peaks = [float(x) for x in lines[:4]]
        bounces = int(float(lines[4].split()[1]))
        assert bounces >= 4
        assert peaks[0] == pytest.approx(2.0, abs=0.01)
        for before, after in zip(peaks, peaks[1:]):
            assert after < before
            assert after / before == pytest.approx(0.64, abs=0.12)
