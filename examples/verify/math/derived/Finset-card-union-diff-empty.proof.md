...
*Built on: union diff empty is union, for difference on Finset, union with empty preserves cardinality, for cardinality on Finset*
...
| | **Proof** | | **Math** |
|:---:|:---|:---:|:---|
| ① | We prove that union diff empty card derived for cardinality on Finset. |  |  |
| ② | We invoke the derived fact governing difference on Finset: union diff empty is union, for difference on Finset (instantiated for s, t). |  |  |
...
`Finset · cardinality · union diff empty card derived`
