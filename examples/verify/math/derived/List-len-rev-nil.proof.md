...
*Built on: reverse preserves length, for length on List, the empty list has length zero, for length on List*
...
| | **Proof** | | **Math** |
|:---:|:---|:---:|:---|
| ① | We prove that reverse of empty has length zero for length on List. |  |  |
...
| ④ | From step 2 and step 3, this implies len(rev(nil)) equals 0. Hence proven. | ④ | $len(rev(nil)) = 0$ |
...
`List · length · reverse of empty has length zero`
