...
> **Goal.** pair(fst(p), snd(p)) = p for p = pair(a, b)
>
> $$\forall a \in \mathbb{N} \forall b \in \mathbb{N}\quad pair(fst(pair(a, b)), snd(pair(a, b))) = pair(a, b)$$

| | **Proof** | | **Math** |
|:---:|:---|:---:|:---|
| ① | We prove that projections recover the pair for pairing on Pair. |  |  |
...
| ⑤ | From step 2, step 3, and step 4, this implies pair(fst(pair(a, b)), snd(pair(a, b))) equals pair(a, b). Hence proven. | ⑤ | $pair(fst(pair(a, b)), snd(pair(a, b))) = pair(a, b)$ |
...
`Pair · pairing · projections recover the pair`
