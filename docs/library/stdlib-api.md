...
> Auto-generated from `lib/stdlib/` on 2026-08-04 by `scripts/gen_stdlib_docs.py`. Per-function docs come from `#` comments immediately above each `export function`.
...
| `async_maxprocs` | `() -> i32` | — |
| `cont_scaffold_available` | `() -> i32` | Probe: delimited-continuation scaffold linked (Flow frames still cannot suspend). |
| `cont_demo_shift` | `() -> i32` | One-shot C-level shift/resume demo (runtime/flow_cont.c). Returns 42 or -1. |
...
| Name | Signature | Docs |
|------|-----------|------|
| `dual_var` | `(x: f32) -> Dual` | Create a dual number for a variable (derivative = 1) |
| `dual_const` | `(x: f32) -> Dual` | Create a dual number for a constant (derivative = 0) |
| `dual_add` | `(a: Dual, b: Dual) -> Dual` | Addition |
| `dual_sub` | `(a: Dual, b: Dual) -> Dual` | Subtraction |
| `dual_mul` | `(a: Dual, b: Dual) -> Dual` | Multiplication (product rule) |
| `dual_div` | `(a: Dual, b: Dual) -> Dual` | Division (quotient rule) |
| `dual_pow` | `(x: Dual, n: f32) -> Dual` | Power (x^n where n is constant) |
| `dual_sq` | `(x: Dual) -> Dual` | Square |
| `dual_sqrt` | `(x: Dual) -> Dual` | Square root |
| `dual_exp` | `(x: Dual) -> Dual` | Exponential |
| `dual_log` | `(x: Dual) -> Dual` | Natural log |
| `dual_sin` | `(x: Dual) -> Dual` | Sine |
| `dual_cos` | `(x: Dual) -> Dual` | Cosine |
| `dual_tan` | `(x: Dual) -> Dual` | Tangent |
| `dual_relu` | `(x: Dual) -> Dual` | ReLU (rectified linear unit) - common in ML |
| `dual_sigmoid` | `(x: Dual) -> Dual` | Sigmoid |
| `dual_tanh` | `(x: Dual) -> Dual` | Tanh |
| `numerical_grad` | `(f_plus: f32, f_minus: f32, epsilon: f32) -> f32` | Compute numerical gradient using finite differences Useful for testing autodiff implementations |
| `dual_val` | `(d: Dual) -> f32` | HELPER: Extract value and gradient from dual |
| `dual_grad` | `(d: Dual) -> f32` | — |
| `dx` | `(x: f32) -> Dual` | Short alias: create dual variable (derivative = 1) |
| `d` | `(x: f32) -> Dual` | Short alias: create dual constant (derivative = 0) |
| `add` | `(a: Dual, b: Dual) -> Dual` | add(Dual, Dual) -> Dual |
| `add` | `(a: Dual, b: f32) -> Dual` | add(Dual, f32) -> Dual |
| `add` | `(a: f32, b: Dual) -> Dual` | add(f32, Dual) -> Dual |
| `sub` | `(a: Dual, b: Dual) -> Dual` | sub(Dual, Dual) -> Dual |
| `sub` | `(a: Dual, b: f32) -> Dual` | sub(Dual, f32) -> Dual |
| `sub` | `(a: f32, b: Dual) -> Dual` | sub(f32, Dual) -> Dual |
| `mul` | `(a: Dual, b: Dual) -> Dual` | mul(Dual, Dual) -> Dual (product rule) |
| `mul` | `(a: Dual, b: f32) -> Dual` | mul(Dual, f32) -> Dual |
| `mul` | `(a: f32, b: Dual) -> Dual` | mul(f32, Dual) -> Dual |
| `ddiv` | `(a: Dual, b: Dual) -> Dual` | div(Dual, Dual) -> Dual (quotient rule) |
| `divs` | `(a: Dual, b: f32) -> Dual` | div(Dual, f32) -> Dual - named 'divs' to avoid C stdlib conflict with div() |
| `rdiv` | `(a: f32, b: Dual) -> Dual` | div(f32, Dual) -> Dual |
| `addc` | `(a: f32, b: Dual) -> Dual` | --- Legacy aliases (for backwards compatibility) --- |
| `rsub` | `(a: f32, b: Dual) -> Dual` | — |
| `smul` | `(a: f32, b: Dual) -> Dual` | — |
| `scale` | `(a: f32, b: Dual) -> Dual` | — |
| `add` | `(a: f32, b: f32) -> f32` | --- Pure scalar versions (for convenience in expressions) --- |
| `sub` | `(a: f32, b: f32) -> f32` | — |
| `mul` | `(a: f32, b: f32) -> f32` | — |
| `neg` | `(a: Dual) -> Dual` | Negate |
| `sigmoid` | `(x: Dual) -> Dual` | Sigmoid (short alias) |
| `ln` | `(x: Dual) -> Dual` | Natural log (short alias) |
| `log` | `(x: Dual) -> Dual` | Alias: log = ln for those who prefer log() |
| `e` | `(x: Dual) -> Dual` | Exponential (short alias) |
| `sinD` | `(x: Dual) -> Dual` | Sine (short alias) - use dual_sin for Duals |
| `cosD` | `(x: Dual) -> Dual` | Cosine (short alias) - use dual_cos for Duals |
| `tanhD` | `(x: Dual) -> Dual` | Tanh (short alias) - use dual_tanh for Duals |
| `sq` | `(x: Dual) -> Dual` | Square: x² |
| `cube` | `(x: Dual) -> Dual` | Cube: x³ |
| `pow4` | `(x: Dual) -> Dual` | Fourth power: x⁴ |
| `sq_diff` | `(x: Dual, c: f32) -> Dual` | (x - c)² for a constant c |
| `linear` | `(x: Dual, a: f32, b: f32) -> Dual` | a*x + b (linear) |
| `quadratic` | `(x: Dual, a: f32, b: f32, c: f32) -> Dual` | a*x² + b*x + c (quadratic) |
| `sin_scaled` | `(x: Dual, a: f32) -> Dual` | sin(a*x) where a is constant |
| `cos_scaled` | `(x: Dual, a: f32) -> Dual` | cos(a*x) where a is constant |
| `exp_scaled` | `(x: Dual, a: f32) -> Dual` | exp(a*x) where a is constant |
| `chain_add` | `(f: Dual, g: Dual) -> Dual` | f + g |
| `chain_mul` | `(f: Dual, g: Dual) -> Dual` | f * g |
| `weighted_sum` | `(f: Dual, a: f32, g: Dual, b: f32) -> Dual` | a*f + b*g (weighted sum) |
| `sum3` | `(a: Dual, b: Dual, c: Dual) -> Dual` | Sum of three terms |
...
| Name | Signature | Docs |
|------|-----------|------|
| `op_add` | `(a: f32, b: f32) -> BinaryResult` | Addition: d(a+b)/da = 1, d(a+b)/db = 1 |
| `op_sub` | `(a: f32, b: f32) -> BinaryResult` | Subtraction: d(a-b)/da = 1, d(a-b)/db = -1 |
| `op_mul` | `(a: f32, b: f32) -> BinaryResult` | Multiplication: d(a*b)/da = b, d(a*b)/db = a |
| `op_div` | `(a: f32, b: f32) -> BinaryResult` | Division: d(a/b)/da = 1/b, d(a/b)/db = -a/b² |
| `op_sq` | `(x: f32) -> UnaryResult` | Square: d(x²)/dx = 2x |
| `op_sqrt` | `(x: f32) -> UnaryResult` | Square root: d(√x)/dx = 1/(2√x) |
| `op_exp` | `(x: f32) -> UnaryResult` | Exponential: d(eˣ)/dx = eˣ |
| `op_log` | `(x: f32) -> UnaryResult` | Log: d(ln x)/dx = 1/x |
| `op_sigmoid` | `(x: f32) -> UnaryResult` | Sigmoid: σ(x) = 1/(1+e⁻ˣ), dσ/dx = σ(1-σ) |
| `op_sin` | `(x: f32) -> UnaryResult` | Sin: d(sin x)/dx = cos x |
| `op_cos` | `(x: f32) -> UnaryResult` | Cos: d(cos x)/dx = -sin x |
| `op_tanh` | `(x: f32) -> UnaryResult` | Tanh: d(tanh x)/dx = 1 - tanh²x |
| `op_relu` | `(x: f32) -> UnaryResult` | ReLU: d(relu x)/dx = 1 if x > 0 else 0 |
| `op_neg` | `(x: f32) -> UnaryResult` | Negation: d(-x)/dx = -1 |
...
| Name | Signature | Docs |
|------|-----------|------|
| `mat_new` | `(rows: i32, cols: i32) -> Mat` | MATRIX CREATION |
| `mat_free` | `(m: Mat) -> void` | — |
| `mat_wrap` | `(data: ptr<f64>, rows: i32, cols: i32) -> Mat` | — |
| `mat_clone` | `(m: Mat) -> Mat` | — |
| `mat_get` | `(m: Mat, i: i32, j: i32) -> f64` | ELEMENT ACCESS (row-major) |
| `mat_set` | `(m: Mat, i: i32, j: i32, val: f64) -> void` | — |
| `gemm` | `(A: Mat, B: Mat, C: Mat) -> void` | BLAS WRAPPERS (ergonomic API) |
...
| `solve` | `(A: Mat, b: ptr<f64>, x: ptr<f64>) -> i32` | LAPACK WRAPPERS |
| `matmul` | `(A: Mat, B: Mat) -> Mat` | CONVENIENCE: MATLAB-STYLE OPERATORS |
| `transpose` | `(A: Mat) -> Mat` | — |
| `eye` | `(n: i32) -> Mat` | — |
...
| `log` | `(x: f32) -> f32` | — |
| `exp` | `(x: f32) -> f32` | — |
| `fibonacci` | `(n: i32) -> i32` | Advanced mathematical functions |
| `gcd` | `(a: i32, b: i32) -> i32` | — |
| `lcm` | `(a: i32, b: i32) -> i32` | — |
| `is_prime` | `(n: i32) -> bool` | — |
