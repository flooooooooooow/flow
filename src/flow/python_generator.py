...
ABI_TYPE_MAP: Dict[str, ABIType] = {
    ...
    "u32": ABIType(
        ...
        "(uint32_t)PyLong_AsUnsignedLong({val})",
        "I",
    ),
    "u64": ABIType(
        "u64",
        ...
        "(uint64_t)PyLong_AsUnsignedLongLong({val})",
        "K",
    ),
    # Floats
    "f32": ABIType(
    ...
    # Boolean
    "bool": ABIType(
        "bool", "bool", "int", "PyBool_FromLong({val})", "PyObject_IsTrue({val})", "p"
    ),
    # String
    "string": ABIType(
        "string",
    ...
    # Void
    "void": ABIType("void", "None", "void", "Py_None", "", ""),
}


def is_abi_compatible(
    flow_type: Type, structs: Dict[str, StructDecl]
) -> Tuple[bool, str]:
    """
...
def infer_exports(
    ...
    for fn in functions:
        ...
        if fn.name == "main":
            result.diagnostics.append(
                ExportDiagnostic(fn.name, "excluded", "Entry point 'main' not exported")
            )
            continue

        # Export this function
        result.exports.append(
            ExportedSymbol(name=fn.name, python_name=fn.name, kind="function", decl=fn)
        )
        result.diagnostics.append(
            ExportDiagnostic(
    ...
    for name, struct in structs.items():
        if not is_public_symbol(name):
            result.diagnostics.append(
                ExportDiagnostic(name, "excluded", "Private struct")
            )
            continue
        ...
        if compat:
            result.exports.append(
                ExportedSymbol(name=name, python_name=name, kind="struct", decl=struct)
            )
            result.diagnostics.append(
                ExportDiagnostic(
    ...
    for const in consts:
        ...
        if compat:
            result.exports.append(
                ExportedSymbol(
                    name=const.name, python_name=const.name, kind="const", decl=const
                )
            )
...
class PythonBindingGenerator:
    ...
    def __init__(
        self, module_name: str, exports: ExportResult, structs: Dict[str, StructDecl]
    ):
...
"""
    ...
    def generate_function_wrapper(self, sym: ExportedSymbol) -> str:
        ...
        for param in fn.parameters:
            type_name = (
                param.type.name if isinstance(param.type, Type) else str(param.type)
            )
            if type_name in ABI_TYPE_MAP:
        ...
        if fn.parameters:
            param_suffix = "_".join(
                (p.type.name if isinstance(p.type, Type) else str(p.type))
                .replace("<", "_")
                .replace(">", "")
                .replace(",", "_")
