...
class GPUCodeGenerator:
    ...
    def _detect_best_backend(self) -> str:
        ...
        # Check for CUDA
        if self.gpu_runtime.is_available() and "cuda" in self.gpu_runtime.list_backends():
            return "cuda"
        
        # Check for OpenCL
        if self.gpu_runtime.is_available() and "opencl" in self.gpu_runtime.list_backends():
            return "opencl"
        ...
        for param in function.parameters:
            if hasattr(param.type, 'name'):
                if param.type.name == 'array_f32':
                    ...
                    params.append(f"{self._flow_to_cuda_type(param.type.name)} {param.name}")
                else:
        ...
        for param in function.parameters:
            if hasattr(param.type, 'name'):
                if param.type.name == 'array_f32':
                    ...
                    params.append(f"{self._flow_to_opencl_type(param.type.name)} {param.name}")
                else:
...
class GPUCompiler:
    ...
    def compile_gpu_kernel(self, function: FunctionDecl, backend: str = "auto") -> Optional[Any]:
    ...
    def _compile_metal_kernel(self, kernel_code: str, kernel_name: str) -> Optional[Any]:
    ...
    def _compile_cuda_kernel(self, kernel_code: str, kernel_name: str) -> Optional[Any]:
        ...
        if not self.gpu_runtime.is_available() or "cuda" not in self.gpu_runtime.list_backends():
    ...
    def _compile_opencl_kernel(self, kernel_code: str, kernel_name: str) -> Optional[Any]:
        """Compile OpenCL kernel."""
        if not self.gpu_runtime.is_available() or "opencl" not in self.gpu_runtime.list_backends():
...
class GPUExecutor:
    ...
    def execute_gpu_function(self, function: FunctionDecl, args: List[Any], backend: str = "auto") -> Optional[Any]:
        """Execute a FLOW function on GPU."""
        if backend == "auto":
            backend = self.code_generator._detect_best_backend()
        
        if backend == "metal" and self.metal_runtime and self.metal_runtime.is_available():
...
class GPUIntegration:
    ...
    def get_gpu_info(self) -> Dict[str, Any]:
        ...
        if self.gpu_runtime.is_available():
            if not info.get("available"):
                info["available"] = True
                info["primary_backend"] = "cuda" if "cuda" in self.gpu_runtime.list_backends() else "opencl"
    ...
    def compile_and_execute(self, function: FunctionDecl, args: List[Any], backend: str = "auto") -> Optional[Any]:
...
def gpu_execute_function(function: FunctionDecl, args: List[Any], backend: str = "auto") -> Optional[Any]:
