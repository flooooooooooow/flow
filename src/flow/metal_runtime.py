...
class MetalShaderCompiler:
    ...
    def _find_xcrun(self) -> Optional[str]:
        ...
        # Try common paths
        common_paths = ["/usr/bin/xcrun", "/Developer/usr/bin/xcrun"]
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    [self.xcrun_path, "metal", "-c", metal_file, "-o", air_file],
                    capture_output=True,
                    text=True,
                )
                ...
                result = subprocess.run(
                    [self.xcrun_path, "metallib", air_file, "-o", lib_file],
                    capture_output=True,
                    text=True,
                )
        ...
        for param in function.parameters:
            if hasattr(param.type, "name"):
                if param.type.name == "array_f32":
                    ...
                    params.append(
                        f"{metal_type} {param.name} [[thread_position_in_grid]]"
                    )
                else:
