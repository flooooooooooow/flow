...
class TypeChecker:
    ...
    def _check_expression(self, expr: Any) -> SemanticType:
        ...
        if isinstance(expr, Literal):
            ...
            if then_type.kind != else_type.kind and then_type.kind != TypeKind.UNKNOWN and else_type.kind != TypeKind.UNKNOWN:
                # Allow numeric widening via existing cast rules when kinds differ lightly.
                if not (self._is_numeric(then_type) and self._is_numeric(else_type)):
                    self.errors.append(
                        f"if-expression branches must have the same type, got {then_type} and {else_type}"
                    )
            return then_type
