...
class CUDABackend(GPUBackend):
    ...
    def _setup_cuda_functions(self):
        ...
        # cudaMalloc
        self.cuda_lib.cudaMalloc.argtypes = [
...
class OpenCLBackend(GPUBackend):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            result = self.opencl_lib.clGetPlatformIDs(
                0, None, ctypes.byref(platform_count)
            )
            if result != 0:
                return False
...
def gpu_copy_to_device(
    host_data: np.ndarray, device_ptr: int, backend: str = "cuda"
) -> None:
    """Copy data from host to device."""
    runtime = get_gpu_runtime()
    gpu_backend = runtime.get_backend(backend)
...
def gpu_copy_from_device(
    device_ptr: int, host_data: np.ndarray, backend: str = "cuda"
) -> None:
    """Copy data from device to host."""
    runtime = get_gpu_runtime()
    gpu_backend = runtime.get_backend(backend)
