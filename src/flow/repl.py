...
class FlowREPL:
    ...
    def print_banner(self):
        ...
        print(
            f"Type expressions, statements, or {self.YELLOW}:help{self.RESET} for commands"
        )
        print(
            f"Press {self.YELLOW}Ctrl+D{self.RESET} or type {self.YELLOW}:quit{self.RESET} to exit"
        )
        print()
...
""")
    ...
    def add_function(self, func_code: str):
        ...
        try:
            ...
            if decls and isinstance(decls[0], FunctionDecl):
                name = decls[0].name
                params = ", ".join(
                    [f"{p.name}: {p.type.name}" for p in decls[0].parameters]
                )
                ret = decls[0].return_type.name
                print(
                    f"{self.GREEN}Defined function {name}({params}) -> {ret}{self.RESET}"
                )
        except Exception as e:
    ...
    def add_struct(self, struct_code: str):
        ...
        try:
            ...
            if decls and isinstance(decls[0], StructDecl):
                name = decls[0].name
                fields = ", ".join(
                    [f"{f.name}: {f.type.name}" for f in decls[0].fields]
                )
    ...
    def add_variable(self, var_code: str):
        ...
        try:
            ...
            if "=" in rest:
                ...
                print(
                    f"{self.RED}Variable declaration needs an initializer{self.RESET}"
                )
    ...
    def compile_and_run_expr(self, expr: str) -> Optional[Any]:
        ...
        try:
            ...
            result = subprocess.run(
                [str(temp_exe)], capture_output=True, text=True, timeout=5
            )
