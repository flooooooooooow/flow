    ...
    def run_file(self, file_path: str) -> TestResult:
        ...
        try:
            ...
            transpile_result = subprocess.run(
                [sys.executable, '-m', 'flow.transpiler', str(file_path), '--c', '--lenient', '-o', str(c_file)],
                capture_output=True,
                text=True,
    ...
    try:
        ...
        if passed == total:
            ...
            if not verbose:
                ...
                for r in results:
                    if not r.passed:
                        print(f"  - {r.name}: {r.error[:80] if r.error else f'exit code {r.exit_code}'}")
            return 1
    finally:
        runner.cleanup()
...
def main():
    ...
    parser.add_argument("path", nargs="?", default="tests/runtime",
