...
def parse_physics_dsl(code: str) -> PhysicsWorld:
    ...
    # Parse constraints
    constraints = []
    constraint_pattern = r'constraint\s+"([^"]+)"\s+when\s+([^:]+):\s*\n((?:\s+[^\n]+\n?)+)'
    ...
    for match in re.finditer(interact_pattern, code):
        interactions.append(PhysicsInteraction(
            match.group(1),
...
def compile_physics_to_flow(world: PhysicsWorld) -> str:
    ...
    lines.append("function vec2_add(a_x: f64, a_y: f64, b_x: f64, b_y: f64, out: ptr<f64>) -> void {")
    ...
    for entity in world.entities:
        ...
        lines.append(f"# Fields per {entity.name}: {len(entity.fields) * 2} f64s (Vec2 = 2 f64)")
        
        # Accessor functions
        for i, (fname, ftype, default) in enumerate(entity.fields):
            if ftype == 'Vec2':
                offset = i * 2
                lines.append(f"function get_{entity.name.lower()}_{fname}_x(data: ptr<f64>, idx: i32) -> f64 {{")
                lines.append(f"    return data[idx * {len(entity.fields) * 2} + {offset}]")
                lines.append("}")
                lines.append(f"function get_{entity.name.lower()}_{fname}_y(data: ptr<f64>, idx: i32) -> f64 {{")
                lines.append(f"    return data[idx * {len(entity.fields) * 2} + {offset + 1}]")
                lines.append("}")
                lines.append(f"function set_{entity.name.lower()}_{fname}(data: ptr<f64>, idx: i32, x: f64, y: f64) -> void {{")
                lines.append(f"    data[idx * {len(entity.fields) * 2} + {offset}] = x")
                lines.append(f"    data[idx * {len(entity.fields) * 2} + {offset + 1}] = y")
                lines.append("}")
            else:
                offset = i * 2
                lines.append(f"function get_{entity.name.lower()}_{fname}(data: ptr<f64>, idx: i32) -> f64 {{")
                lines.append(f"    return data[idx * {len(entity.fields) * 2} + {offset}]")
                lines.append("}")
                lines.append(f"function set_{entity.name.lower()}_{fname}(data: ptr<f64>, idx: i32, val: f64) -> void {{")
                lines.append(f"    data[idx * {len(entity.fields) * 2} + {offset}] = val")
                lines.append("}")
        lines.append("")
    ...
    if entity:
        ...
        lines.append(f"function update_{entity.name.lower()}s(data: ptr<f64>, count: i32, dt: f64, width: f64, height: f64) -> void {{")
        lines.append("    for i in 0 to count {")
        
        # Load current values
        for fname, ftype, _ in entity.fields:
            if ftype == 'Vec2':
                lines.append(f"        let {fname}_x: f64 = get_{entity.name.lower()}_{fname}_x(data, i)")
                lines.append(f"        let {fname}_y: f64 = get_{entity.name.lower()}_{fname}_y(data, i)")
        ...
        for fname, ftype, _ in entity.fields:
            if ftype == 'Vec2':
                lines.append(f"        set_{entity.name.lower()}_{fname}(data, i, {fname}_x, {fname}_y)")
