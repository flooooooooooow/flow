...
class MetalCodegen:
    ...
    def map_type(self, flow_type: Type, is_buffer: bool = True) -> str:
        ...
        if type_name.startswith('array_') or (hasattr(flow_type, 'element_type') and flow_type.element_type):
        ...
        if type_name.startswith('array<') or type_name.startswith('ptr<'):
            inner = type_name[6:-1] if type_name.startswith('array<') else type_name[4:-1]
            metal_inner = self.TYPE_MAP.get(inner, inner)
            return f"device {metal_inner}*"
    ...
    def is_array_type(self, flow_type: Type) -> bool:
        ...
        return (flow_type.name.startswith('array<') or 
                flow_type.name.startswith('array_') or 
                flow_type.name.startswith('ptr<') or
                (hasattr(flow_type, 'element_type') and flow_type.element_type is not None))
    ...
    def generate_kernel(self, func: FunctionDecl) -> str:
        ...
        for param in func.parameters:
            ...
            if self.is_array_type(param.type):
                ...
                self.buffer_bindings[param.name] = binding
                self.binding_counter += 1
                params.append(f"constant {metal_type}& {param.name} [[buffer({binding})]]")
    ...
    def generate_assignment(self, assign: Assignment) -> List[str]:
        ...
        if assign.target_expr:
            ...
            target = assign.target if isinstance(assign.target, str) else self.generate_expression(assign.target)
    ...
    def generate_expression(self, expr: Expression) -> str:
        ...
        if isinstance(expr, Literal):
            ...
            # Math functions
            math_funcs = ['sin', 'cos', 'tan', 'sqrt', 'exp', 'log', 'abs', 'min', 'max', 'pow']
    ...
    def generate_host_code(self, func: FunctionDecl, shader_path: str) -> str:
        ...
        host_params = []
        for param in params:
            if param.type.name.startswith('array<') or param.type.name.startswith('ptr<'):
                inner = param.type.name[6:-1] if param.type.name.startswith('array<') else param.type.name[4:-1]
                c_type = {'f32': 'float', 'f64': 'double', 'i32': 'int', 'u32': 'unsigned int'}.get(inner, 'float')
                host_params.append(f"{c_type}* {param.name}")
            else:
                c_type = {'f32': 'float', 'f64': 'double', 'i32': 'int', 'u32': 'unsigned int', 'bool': 'bool'}.get(param.type.name, 'int')
                host_params.append(f"{c_type} {param.name}")
        ...
        code.append("        NSString* shaderSource = [NSString stringWithContentsOfFile:shaderPath encoding:NSUTF8StringEncoding error:&error];")
        code.append("        id<MTLLibrary> library = [device newLibraryWithSource:shaderSource options:nil error:&error];")
        code.append("        if (!library) {")
        code.append('            NSLog(@"Failed to compile shader: %@", error);')
        code.append("            return;")
        code.append("        }")
        code.append("")
        code.append(f'        id<MTLFunction> kernel = [library newFunctionWithName:@"{func.name}"];')
        code.append("        id<MTLComputePipelineState> pipeline = [device newComputePipelineStateWithFunction:kernel error:&error];")
        code.append("")
        code.append("        // Create command queue")
        code.append("        id<MTLCommandQueue> queue = [device newCommandQueue];")
        code.append("        id<MTLCommandBuffer> commandBuffer = [queue commandBuffer];")
        code.append("        id<MTLComputeCommandEncoder> encoder = [commandBuffer computeCommandEncoder];")
        ...
        for i, param in enumerate(params):
            if param.type.name.startswith('array<') or param.type.name.startswith('ptr<'):
                inner = param.type.name[6:-1] if param.type.name.startswith('array<') else param.type.name[4:-1]
                c_type = {'f32': 'float', 'f64': 'double', 'i32': 'int', 'u32': 'unsigned int'}.get(inner, 'float')
                code.append(f"        id<MTLBuffer> buffer{i} = [device newBufferWithBytes:{param.name} length:count * sizeof({c_type}) options:MTLResourceStorageModeShared];")
                code.append(f"        [encoder setBuffer:buffer{i} offset:0 atIndex:{i}];")
            else:
                code.append(f"        [encoder setBytes:&{param.name} length:sizeof({param.name}) atIndex:{i}];")
        
        code.append("")
        code.append("        // Dispatch threads")
        code.append("        MTLSize gridSize = MTLSizeMake(count, 1, 1);")
        code.append("        NSUInteger threadGroupSize = MIN(pipeline.maxTotalThreadsPerThreadgroup, count);")
        code.append("        MTLSize threadgroupSize = MTLSizeMake(threadGroupSize, 1, 1);")
        code.append("        [encoder dispatchThreads:gridSize threadsPerThreadgroup:threadgroupSize];")
        code.append("")
        ...
        for i, param in enumerate(params):
            if 'out' in param.name.lower() or 'result' in param.name.lower():
                inner = param.type.name[6:-1] if param.type.name.startswith('array<') else param.type.name[4:-1]
                c_type = {'f32': 'float', 'f64': 'double', 'i32': 'int', 'u32': 'unsigned int'}.get(inner, 'float')
                code.append(f"        memcpy({param.name}, buffer{i}.contents, count * sizeof({c_type}));")
...
def generate_metal_shaders(declarations: List[Any], output_dir: str = "build/gpu") -> List[Tuple[str, str]]:
