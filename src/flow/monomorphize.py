...
class Monomorphizer:
    ...
    def _request_function_mono(self, call: FunctionCall) -> None:
        ...
        if (
            hasattr(call, "type_args")
            and call.type_args
            and len(call.type_args) == num_type_params
        ):
        ...
        for arg, param in zip(call.arguments, original.parameters):
            self._infer_type_arg(
                arg, param.type, generic_def.type_params, type_param_map
            )
    ...
    def _infer_type_arg(
        self, arg: Expression, param_type: Type, type_params: list, result: dict
    ) -> None:
    ...
    def _generate_function_inner(
        self, req: MonomorphRequest, generic_def: GenericDef
    ) -> None:
        original = generic_def.decl
        type_map = dict(zip(generic_def.type_params, req.type_args))
    ...
    def _substitute_statement(
        self, stmt: Statement, type_map: Dict[str, Type]
    ) -> Statement:
        """Substitute types in a statement."""
        if isinstance(stmt, VarDecl):
            new_type = self._substitute_type(stmt.type, type_map)
            new_init = (
                self._substitute_expression(stmt.initializer, type_map)
                if stmt.initializer
                else None
            )
            return VarDecl(
            ...
            new_value = (
                self._substitute_expression(stmt.value, type_map)
                if stmt.value
                else None
            )
            ...
            new_target_expr = (
                self._substitute_expression(stmt.target_expr, type_map)
                if stmt.target_expr
                else None
            )
            ...
            new_elifs = [
                (
                    self._substitute_expression(c, type_map),
                    self._substitute_block(b, type_map),
                )
                for c, b in stmt.elif_blocks
            ]
            new_else = (
                self._substitute_block(stmt.else_block, type_map)
                if stmt.else_block
                else None
            )
            ...
            new_step = (
                self._substitute_expression(stmt.step, type_map) if stmt.step else None
            )
            new_body = self._substitute_block(stmt.body, type_map)
            return ForStatement(
                stmt.variable, new_start, new_end, new_step, new_body, stmt.is_parallel
            )
        elif isinstance(stmt, Block):
    ...
    def _substitute_expression(
        self, expr: Expression, type_map: Dict[str, Type]
    ) -> Expression:
        """Substitute types in an expression."""
        if isinstance(expr, FunctionCall):
            new_args = [
                self._substitute_expression(a, type_map) for a in expr.arguments
            ]
            ...
            new_fields = [
                (n, self._substitute_expression(v, type_map)) for n, v in expr.fields
            ]
            ...
            new_elems = [
                self._substitute_expression(e, type_map) for e in expr.elements
            ]
    ...
    def _rewrite_statement(self, stmt: Statement) -> Statement:
        ...
        if isinstance(stmt, VarDecl):
            ...
            new_init = (
                self._rewrite_expression(stmt.initializer) if stmt.initializer else None
            )
            return VarDecl(
                stmt.name,
                new_type,
            ...
            new_target_expr = (
                self._rewrite_expression(stmt.target_expr) if stmt.target_expr else None
            )
            return Assignment(stmt.target, new_value, new_target_expr)
        elif isinstance(stmt, IfStatement):
            ...
            new_elifs = [
                (self._rewrite_expression(c), self._rewrite_block(b))
                for c, b in stmt.elif_blocks
            ]
