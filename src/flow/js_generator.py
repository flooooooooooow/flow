...
class JSGenerator:
    ...
    def generate(self, declarations: List) -> str:
        ...
        self.emit_raw(
            "    println: function(s) { this.output.push(String(s) + '\\n'); },"
        )
        ...
        self.emit_raw(
            "        return { success: true, output: __flow_runtime.getOutput(), result: result };"
        )
        self.emit_raw("    } catch (e) {")
        self.emit_raw(
            "        return { success: false, output: __flow_runtime.getOutput(), error: e.toString() };"
        )
        self.emit_raw("    }")
        self.emit_raw("}")
    ...
    def _gen_stmt(self, stmt):
        ...
        if isinstance(stmt, VarDecl):
            value = (
                self._gen_expr(stmt.initializer) if stmt.initializer else "undefined"
            )
    ...
    def _gen_expr(self, expr) -> str:
        ...
        if isinstance(expr, Literal):
            ...
            func_name = (
                expr.name if isinstance(expr.name, str) else self._gen_expr(expr.name)
            )
            ...
            name = (
                expr.struct_name.split("<")[0]
                if hasattr(expr, "struct_name")
                else str(expr.name).split("<")[0]
            )
            field_values = []
