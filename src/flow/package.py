...
class FlowPackageManager:
    ...
    def init(self, name: Optional[str] = None) -> bool:
        ...
        if self.config_file.exists():
            print(
                f"{self.YELLOW}Project already initialized (flow.toml exists){self.RESET}"
            )
            return False
...
""")
    ...
    def install_package(self, package_name: str, version: str = "*") -> bool:
        ...
        print(
            f"{self.YELLOW}Package {package_name} will be resolved at compile time{self.RESET}"
        )
        return True
    ...
    def build(self, release: bool = False) -> bool:
        ...
        _output_file = build_dir / output_name
    ...
    def build_native(
        self, entry_file: Optional[str] = None, release: bool = False
    ) -> bool:
        ...
        try:
            env = os.environ.copy()
            env["PYTHONPATH"] = (
                f"{repo_root / 'src'}{os.pathsep}{env.get('PYTHONPATH', '')}".rstrip(
                    os.pathsep
                )
            )
        ...
        for native_src in config.native_sources:
            ...
            if native_path.exists():
                ...
                print(
                    f"{self.YELLOW}Warning: native source not found: {native_src}{self.RESET}"
                )

        # Use C++ compiler if any C++ sources are present
        uses_cxx = any(
            str(src).endswith((".cpp", ".cc", ".cxx", ".mm")) for src in sources
        )
        ...
        try:
            if uses_cxx:
                ...
                c_compile = (
                    ["clang"] + c_flags_c + ["-c", str(c_file), "-o", str(c_obj)]
                )
                ...
                for idx, src in enumerate(sources[1:]):
                    ...
                    result = subprocess.run(
                        native_compile, capture_output=True, text=True
                    )
                    if result.returncode != 0:
                        print(f"{self.RED}Native compilation failed:{self.RESET}")
                        print(result.stderr)
    ...
    def run_native(self, entry_file: Optional[str] = None) -> bool:
        ...
        try:
            ...
            try:
                print(
                    f"{self.YELLOW}Shutdown requested (SIGINT). Waiting for app to exit...{self.RESET}"
                )
...
def main():
    ...
    build_parser.add_argument(
        "--release", "-r", action="store_true", help="Release build"
    )

    # build-native
    build_native_parser = subparsers.add_parser(
        "build-native", help="Build with native source support"
    )
    ...
    # run-native
    run_native_parser = subparsers.add_parser(
        "run-native", help="Build and run with native support"
    )
