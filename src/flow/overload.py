...
class OverloadResolver:
    ...
    def register_function(self, func: FunctionDecl):
        ...
        entry = OverloadEntry(
            function=func, param_types=param_types, mangled_name=mangled
        )
    ...
    def resolve_call(self, call: FunctionCall) -> Optional[str]:
        ...
        if name in self.C_MATH_FUNCTIONS:
            ...
            all_primitive = all(at is None or at in self.PRIMITIVES for at in arg_types)
            ...
            if len(entry.param_types) == len(arg_types):
                ...
                for param_type, arg_type in zip(entry.param_types, arg_types):
                    if arg_type is None:
                        ...
                        break
                    if param_type != arg_type and not self._types_compatible(
                        param_type, arg_type
                    ):
                        all_compatible = False
                        break
