...
class MLIRGenerator:
    ...
    def _format_mlir_numeric(value: str, mlir_type: str) -> str:
        ...
        if mlir_type in ("f32", "f64") and ("e" in lowered):
            try:
                ...
                return value
        # Large unsigned integer literals that exceed signed i32 range
        # must be converted to their two's complement representation,
        # otherwise MLIR truncates them to 0.
        if mlir_type == "i32":
            try:
                n = int(value)
                if n > 2147483647:
    ...
    def generate_const(self, const: ConstDecl) -> str:
        ...
        # Handle CastExpression wrapping a Literal (e.g. `2147483648 as u32`)
        # and UnaryOperation negation on a Literal (e.g. `-1073741824`).
        unwrapped = const.value
        if isinstance(unwrapped, CastExpression) and isinstance(unwrapped.expr, Literal):
        ...
        if isinstance(unwrapped, Literal) and getattr(unwrapped.type, "name", "") != "string":
            if mlir_type == "!llvm.ptr" or str(unwrapped.value).lower() == "null":
                ...
                return "\n".join(mlir_code)
            literal_value = self._format_mlir_numeric(str(unwrapped.value), mlir_type)
            if const.type.name == 'bool':
                literal_value = "1" if str(unwrapped.value).lower() in ("true", "1") else "0"
            mlir_code.append(
                f"{self.indent()}llvm.mlir.global internal constant @{const.name}"
                f"({literal_value} : {mlir_type}) : {mlir_type}"
            )
