...
class FlowJITRunner:
    ...
    def compile_and_run(self):
        ...
        try:
            ...
            env["PYTHONPATH"] = (
                src_path if not existing_pp else f"{src_path}:{existing_pp}"
            )
    ...
    def start_hot_reload(self):
        ...
        if _HAS_WATCHDOG:
            ...
            observer = Observer()
            observer.schedule(
                FlowFileHandler(self), str(self.watch_dir), recursive=True
            )
            observer.start()
...
def main():
    ...
    parser.add_argument(
        "--watch", help="Directory to watch for changes (default: file directory)"
    )
    parser.add_argument(
        "--once", action="store_true", help="Run once without hot reload"
    )
