...
| Update examples with verified status | ✅ | `examples/STATUS.md` (891/1170 compile, 76.2%), regenerated by `scripts/verify_examples.py` |
...
| Targeted fuzzing harness | ✅ | `tests/fuzz/` (mutation/grammar/pipeline targets, seeded, auto-shrink); 2 known crashes recorded as xfail regressions |
...
- 2 known parser crashes found by fuzzing, recorded as xfail regressions in
  `tests/fuzz/test_crash_regressions.py`: `ValueError` in `parse_type` on a float
  array size (`[i32; 0.0]`), and `RecursionError` on deeply nested expressions
  (~70 nested parens at the default limit)
...
- 279 of 1170 swept files still fail compile-check (271 parser — mostly the
  `examples/verify/` proof corpus — and 8 C-codegen bugs); see
  [examples/STATUS.md](examples/STATUS.md) for the per-file breakdown
...
7. 🔲 **Fix the fuzz crashes** - the two xfail parser crashes in `tests/fuzz/test_crash_regressions.py`
8. 🔲 **Remaining codegen bugs** - the 8 cgen failures listed in `examples/STATUS.md`
