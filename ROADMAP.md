...
| Phase | Focus | Status |
|-------|--------|--------|
| **W0** | RF units + quantity literals + complex/IQ + phantom rate `Signal` + memory/RT attrs + docs/examples | 🔨 partial: `c64`/`c128` complex types (C99 `_Complex`), `c64()`/`c128()` constructors, `creal`/`cimag`/`cabs`/`carg`/`conj`/`cexp`/`clog`/`csqrt`/`cpow` builtins, complex arithmetic with auto-promotion, `lib/stdlib/rf.flow` (`IQ` alias, `IQSample` distinct type, `Signal` struct with sample_rate, `signal_new`/`signal_mix`/`signal_scale`/`signal_magnitude_spectrum`), `lib/stdlib/units_si.flow` (SI base + derived units), quantity literals (`3.14 Hertz` desugars to `3.14 as Hertz`), `examples/rf/dft.flow` + `examples/rf/iq_mixer.flow` + `examples/rf/sdr_receiver.flow`, `tests/unit/test_complex_types.py` (15) + `test_rf_types.py` (10) + `test_quantity_literals.py` (10). Next: compile-time phantom rate checking. |
...
| Task | Status | Impact |
|------|--------|--------|
| Fix `ptr[0].field` parser issue | ✅ | Unified postfix chaining (`f()[1].x`, `pts[0].method()`, …) + C codegen fixes; recovered ring_buffer, memory_pool, hash_table, 2048, tetris_gfx, csv_parser, flowdb |
| Add GitHub Actions CI | ✅ | pytest with warnings-as-errors, tier-2 runs `--strict`, fuzz job (30s/target) |
| Update examples with verified status | ✅ | `examples/STATUS.md` (985/1192 compile, 82.6%), regenerated by `scripts/verify_examples.py` |
...
| File | Problem | Action | Status |
|------|---------|--------|--------|
| `examples/effects_demo.mlir` | MLIR file loose in examples root | Relocated out of `examples/` | ✅ |
...
- ✅ **FIXED (#117):** `break`/`continue` now have dedicated `BreakStatement`/
  ...
  old "surfaces as a bare `Variable`" special-case is gone. See
  `tests/unit/test_parser.py` and `tests/runtime/test_control_flow.flow`.
- ✅ **FIXED (#119):** The three older `examples/effects/` demos
  ...
  [docs/effects-showcase.md](docs/effects-showcase.md)) and run correctly
  with `./flow run`.
...
- ~270 of the parser failures in that sweep are under `examples/verify/`
  ...
  that [verification.md](docs/language/verification.md) lists as design-spec,
  not shipped syntax. Categorized breakdown + triage tool:
  [docs/third-party/flow-verify-parser-status.md](docs/third-party/flow-verify-parser-status.md),
  `scripts/triage_verify_failures.py`
- Match guards, `|` alternation (literal-only), nested literal patterns
  (`Point(0, y)`), and struct-in-struct nesting (`Outer(Inner(x), y)`, any
  depth) all work end-to-end in the C backend. Exhaustiveness is real for
...
- [x] Make the test corpus strict-clean (`./flow test --strict --tier2` passes 215/215; intentional exceptions carry a `# flow:lenient - <reason>` pragma)
...
**Features:**
- [x] Go to definition
- [x] Find references (token-aware; cross-file for top-level symbols across open docs)
...
Covered by the scripted harness `scripts/test_lsp_server.py` (39 tests).
