...
| Update examples with verified status | ✅ | `examples/STATUS.md` (906/1177 compile, 77.0%), regenerated by `scripts/verify_examples.py` |
...
| Cross-platform graphics (Linux) | 🔲 stub — [docs/language/graphics.md](docs/language/graphics.md) | `gfx_linux.c` ABI-aligned stub; SDL2 backend still needed |
...
| RT-safety policy (no-alloc audio thread) | ✅ partial — [docs/library/rt-safety.md](docs/library/rt-safety.md) | Policy + checklist documented; not compile-time enforced yet |
...
- `break`/`continue` have no dedicated AST nodes — they lex as identifiers and are
  special-cased in the C generator (works, but fragile)
- The three older `examples/effects/` demos (`dependency_injection`, `state_effects`,
  `async_effects`) use the `capability EffectName` parameter style, which transpiles
  but does not link (see [docs/effects-showcase.md](docs/effects-showcase.md))
...
- Match guards, `|` alternation (literal-only), nested literal patterns
  ...
  its `.tag` field both work end-to-end in the C backend). Still a minimal
  integer-literal warning stub for plain integers (`TypeCheckResult.warnings`)
  — no range/gap analysis there
...
- [x] Add nested patterns — literal values in struct-pattern field positions, e.g. `Point(0, y)` requires field 0 == 0 and binds field 1 to `y`; **and** struct-in-struct nesting to arbitrary depth, e.g. `Outer(Inner(x), y)` recursively matches/binds through nested structs (parser: `StructPattern.field_patterns`; codegen: recursive `_gen_struct_pattern_match`; type checker: recursive `_bind_struct_pattern`). See `tests/core/test_match_nested.flow`. Not covered: nesting inside `|` alternation (alternatives must still be plain literals) and enum-variant patterns (see exhaustiveness note below)
- [x] Add `|` for multiple patterns (narrow form) — literal-only alternation, e.g. `1 | 2 | 3 => ...`; combines with guards and generates stacked `case` labels when eligible for a C `switch`
- [x] Exhaustiveness checking — **real** (not stub) for `bool`: since `bool` has exactly two inhabitants, covering both `true` and `false` across arms is genuine exhaustiveness even without a wildcard, and a match covering only one is flagged. **Real** for enum/ADT variants too: a bare identifier match arm that names a known enum-variant constant (e.g. `Color_Red`, `Option_i32_Some`) is now recognized as a path/const pattern — a value-equality check, not an always-binding identifier — via `_lookup_const_symbol`/`_check_match_stmt`'s `Variable` handling; `_warn_enum_exhaustiveness` requires every variant covered or a wildcard/`default`, for both `match <enum value> { ... }` and `match <enum value>.tag { ... }`. Coverage is only attributed when *every* arm is a recognized variant pattern (or wildcard) — any other pattern shape (plain binding, struct pattern, literal) makes the checker bail out rather than risk a false positive, and a guarded arm doesn't count as covering its variant. The C backend lowers these path/const patterns to real tag-equality checks (`_enum_variant_owner` in `c_generator.py`), so "reported exhaustive" now matches actual runtime behavior — see `examples/generics_traits/enum_match_exhaustive.flow`. For integers, still a **minimal stub**: warns when a match has no `_`/`default` and only integer literal arms, but does no range/gap analysis (can't tell "covers all `i8` values" from "covers 2 of 256"). Warnings collected on `TypeCheckResult.warnings` (`tests/unit/test_match_exhaustiveness.py`)
...
9. [x] Match guards + `|` alternation + nested literal & struct-in-struct patterns ✅ (bool and enum/ADT exhaustiveness are real via path/const patterns, integer coverage still a stub; see Phase 2.1)
10. [x] Manual memory stdlib (real heap) ✅ (`lib/stdlib/memory.flow`)
11. [x] RT-safety policy for audio thread (no-alloc contract) ✅ partial — [docs/library/rt-safety.md](docs/library/rt-safety.md) (policy documented; compile-time enforcement still open)
