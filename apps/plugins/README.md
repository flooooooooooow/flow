...
```bash
cmake -S apps/plugins/SchurPhaser -B build/schur -DCMAKE_BUILD_TYPE=Release -DQUILIO_SDK=/path/to/QuilioSDK
...
- The DSP correctness test runs in CI (`plugin-dsp` job) whenever `apps/plugins/**` changes; full plugin builds are not yet in CI
