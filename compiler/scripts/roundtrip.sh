...
# Frontend cgen bundle: FLOWC_BUNDLE=1 emits token+ast+cgen in one TU
# (deps first). No flowc_c_to_hdr.py / -include — same dogfood as lexer/parser.
echo "=== FLOWC_BUNDLE cgen (token+ast+cgen one TU) ==="
FLOWC_FORCE_HOST=1 FLOWC_BUNDLE=1 FLOWC_DIR=compiler/src \
FLOWC_TYPECHECK=0 \
    stage_a_emit compiler/src/cgen.flow compiler/build/bundle_cgen.c
if ! grep -Fq 'flowc_cgen_emit' compiler/build/bundle_cgen.c; then
    echo "FAIL FLOWC_BUNDLE cgen: expected flowc_cgen_emit from cgen.flow" >&2
    exit 1
fi
if ! grep -Fq 'typedef struct AstNode' compiler/build/bundle_cgen.c; then
    echo "FAIL FLOWC_BUNDLE cgen: expected AstNode from ast.flow in bundled C" >&2
    exit 1
fi
if ! grep -Fq 'typedef struct Token' compiler/build/bundle_cgen.c; then
    echo "FAIL FLOWC_BUNDLE cgen: expected Token from token.flow in bundled C" >&2
    exit 1
fi
bundle_cgen_stdint=$(grep -c '#include <stdint.h>' compiler/build/bundle_cgen.c || true)
if [[ "$bundle_cgen_stdint" -ne 1 ]]; then
    echo "FAIL FLOWC_BUNDLE cgen: expected exactly one stdint include, got ${bundle_cgen_stdint}" >&2
    exit 1
fi
cc -O0 -c compiler/build/bundle_cgen.c -o compiler/build/bundle_cgen.o
if ! nm compiler/build/bundle_cgen.o | grep -q 'flowc_cgen_emit'; then
    echo "FAIL FLOWC_BUNDLE cgen: flowc_cgen_emit missing from object" >&2
    exit 1
fi
if ! nm compiler/build/bundle_cgen.o | grep -q 'TOK_EOF'; then
    echo "FAIL FLOWC_BUNDLE cgen: TOK_EOF missing from object" >&2
    exit 1
fi
echo "PASS FLOWC_BUNDLE cgen (token+ast+cgen one TU)"
