...
# Same for parser (token+ast+lexer+parser) and cgen (token+ast+cgen):
FLOWC_BUNDLE=1 FLOWC_DIR=compiler/src FLOWC_TYPECHECK=0 \
FLOWC_IN=compiler/src/cgen.flow FLOWC_OUT=compiler/build/bundle_cgen.c \
  ./flow run compiler/src/main.flow
...
Python ports (Claim Coordinates / math prose / premise instantiate): see
[docs/project/python-in-flow.md](../docs/project/python-in-flow.md);
`./compiler/scripts/smoke_math_prose.sh` checks generated C string constants.
...
| `simple_for.flow` | no | old `0..n` range + `printf` |
