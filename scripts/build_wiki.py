...
def write_example_status() -> None:
    """Generate example-status.md from the ledger the CI gate writes.

    The claim "every example in these docs compiles" is only worth making if a
    reader can check it. This turns the ledger into a page: the totals, then
    every page that still has something unproven, so the gaps are as visible as
    the successes.
    """
    ledger_path = DOCS / "generated" / "example-status.json"
    ...
    proven = counts.get("verified", 0) + counts.get("expected-error", 0)

    lines = [
        "# Example status",
        "",
        "> Generated by `scripts/build_wiki.py` from `docs/generated/example-status.json`, "
        "which `scripts/check_doc_examples.py` writes. Do not edit by hand.",
        "",
        f"Every ```` ```flow ```` block in the documentation is compiled in CI: parsed, "
        f"type-checked in strict mode, lowered to C and run through clang. **{proven} of "
        f"{total}** are proven, either by compiling or by failing exactly as the page says "
        "they should. Each block carries its own result in the header above the code.",
        "",
        "| Result | Blocks | Meaning |",
        "|---|---:|---|",
        f"| Compiles | {counts.get('verified', 0)} | Builds from source to object code. |",
        f"| Rejected on purpose | {counts.get('expected-error', 0)} | Tagged `expect-error`; the page is about what the compiler refuses. |",
        f"| Not compiled | {counts.get('ignored', 0)} | Tagged `ignore=` with a written reason: notation, proposed syntax, or something outside the build. |",
        f"| Not compiling yet | {counts.get('unverified', 0)} | Known debt. Recorded so it cannot grow. |",
        "",
        "## How it is enforced",
        "",
        "The ledger records the blocks that do not build today, keyed by a hash of each "
        "block's own text. Editing a block changes its key, so its exemption disappears and "
        "the new text has to compile. Existing debt is inherited; new and edited content is "
        "not. The `doc-examples` job fails on any block that does not build and is not in "
        "the ledger.",
        "",
    ]
    ...
    for b in blocks:
        page = by_page.setdefault(b["path"], {})
        page[b["status"]] = page.get(b["status"], 0) + 1
    outstanding = sorted(
        ((p, c) for p, c in by_page.items() if c.get("unverified")),
        key=lambda kv: -kv[1]["unverified"],
    )
    if outstanding:
        lines += [
            "## Pages with blocks that do not compile yet",
            "",
            "| Page | Not compiling | Total |",
            "|---|---:|---:|",
        ]
        for page, c in outstanding:
            lines.append(f"| `{page}` | {c['unverified']} | {sum(c.values())} |")
        lines.append("")
    else:
        lines += ["## Outstanding", "", "Nothing. Every block is accounted for.", ""]

    (OUT / "project").mkdir(parents=True, exist_ok=True)
    (OUT / "project" / "example-status.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
