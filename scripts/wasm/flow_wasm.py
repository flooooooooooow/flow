...
Usage:
    python scripts/wasm/flow_wasm.py examples/fibonacci.flow -o output/
    python scripts/wasm/flow_wasm.py my_program.flow --run
"""
...
def find_emcc():
    ...
    # Prefer project-local emsdk under third_party/ (legacy ./emsdk still accepted)
    for emsdk_root in (
        SCRIPT_DIR / "third_party" / "emsdk",
        SCRIPT_DIR / "emsdk",
    ):
        emsdk_emcc = emsdk_root / "upstream" / "emscripten" / "emcc"
...
def compile_to_wasm(flow_file: Path, output_dir: Path, run: bool = False):
    ...
    if not emcc:
        ...
        print("     Install with:")
        print("       git clone https://github.com/emscripten-core/emsdk.git third_party/emsdk")
        print("       cd third_party/emsdk && ./emsdk install latest && ./emsdk activate latest")
